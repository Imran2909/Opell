...
window.addEventListener("load", () => {
    ...
    fetch("https://63c64ff64ebaa80285433dad.mockapi.io/apple").then((res) => {
        ...
        .then((result) => {
            ...
            modeldata = result[1].ShopiPhone[0].Allmodels;
            accessoriesdata = result[1].ShopiPhone[1].Accessories;
            let modelsData = (result[1].ShopiPhone[0].Allmodels);
            let accessoriesData = (result[1].ShopiPhone[1].Accessories);
...
sortPrice.addEventListener("change", () => {
    ...
    if (value == "l2h") {
        ...
        filtered.then((res) => {
            ...
            .then((result) => {
                let fetched = (result[1].ShopiPhone[0].Allmodels);
                let fetchedAcc = (result[1].ShopiPhone[1].Accessories);
                console.log(fetched)
                showData(iPhoneModels, fetched)
                showData(iPhoneAccessories, fetchedAcc)
            })
